...
# Ask to overwrite existing config, returns 0 if should write, 1 if skip
confirm_overwrite() {
    local file="$1"
    local name="$2"
    ...
    if [ -f "$file" ]; then
        echo ""
        warn "$name config already exists: $file"
        read -rp "Overwrite? (backup will be created) [y/N]: " choice
        case "$choice" in
            [Yy]|[Yy][Ee][Ss])
                cp "$file" "$file.backup.$(date +%Y%m%d%H%M%S)"
                info "Backed up to $file.backup.*"
                return 0
                ;;
            *)
                info "Skipping $name config"
                return 1
                ;;
        esac
    fi
...
detect_audio_devices() {
    ...
    # Use freedvtnc2 --list-audio-devices if available (preferred)
    # Otherwise fall back to arecord/aplay
    if command -v freedvtnc2 &> /dev/null; then
        echo -e "${BOLD}Audio devices (as seen by freedvtnc2):${NC}"
        echo ""
        echo -e "  ${BOLD}ID${NC}   ${BOLD}Name${NC}                                            ${BOLD}In${NC}   ${BOLD}Out${NC}"
        echo "  ───────────────────────────────────────────────────────────────"

        # Parse freedvtnc2 device list
        while IFS= read -r line; do
            # Skip header line and ALSA warnings
            if [[ "$line" =~ ^[[:space:]]*[0-9]+ ]]; then
                # Extract ID and show the line
                local id=$(echo "$line" | awk '{print $1}')
                echo "  $line"
            fi
        done < <(freedvtnc2 --list-audio-devices 2>/dev/null | grep -v "^ALSA lib" || true)

        echo ""
        echo -e "${YELLOW}Tip:${NC} Look for your radio's USB audio interface."
        echo "     Choose a device with In > 0 for input, Out > 0 for output."
        echo ""
    else
        # Fallback to ALSA tools
        if command -v arecord &> /dev/null; then
            echo -e "${BOLD}Input devices (capture):${NC}"
            arecord -l 2>/dev/null | grep "^card" || warn "No input devices found"
            echo ""
        fi

        if command -v aplay &> /dev/null; then
            echo -e "${BOLD}Output devices (playback):${NC}"
            aplay -l 2>/dev/null | grep "^card" || warn "No output devices found"
            echo ""
        fi

        warn "freedvtnc2 not found - showing ALSA devices."
        echo "Note: Device numbers may differ from what freedvtnc2 uses."
        echo ""
    fi

    # Input device selection
    echo "Select the audio INPUT device (from radio to computer):"
    echo ""

    while true; do
        read -rp "Enter input device ID: " AUDIO_INPUT
    ...
    echo ""
    echo "Select the audio OUTPUT device (from computer to radio):"
    echo ""

    while true; do
        read -rp "Enter output device ID (or 'same' for same as input): " choice
        if [ "$choice" = "same" ]; then
...
EOF
    ...
    # freedvtnc2 config (uses ~/.freedvtnc2.conf)
    if confirm_overwrite "$HOME/.freedvtnc2.conf" "freedvtnc2"; then
        cat > "$HOME/.freedvtnc2.conf" << EOF
...
EOF
        ...
        success "Created: ~/.freedvtnc2.conf"
    fi

    # Reticulum config - add HF interface if not present
    mkdir -p "$HOME/.reticulum"
    local reti_config="$HOME/.reticulum/config"

    # Define the HF interface block
    local hf_interface_block="
  [[HF Radio via FreeDV]]
    ...
    if [ -f "$reti_config" ]; then
        # Config exists - check if HF interface already present
        if grep -q "HF Radio via FreeDV" "$reti_config" 2>/dev/null; then
            info "Reticulum HF interface already configured"
        else
            # Append HF interface to existing config
            info "Adding HF interface to existing Reticulum config..."
            cp "$reti_config" "$reti_config.backup.$(date +%Y%m%d%H%M%S)"
...
EOF
    ...
    # User can configure manually if needed
...
print_summary() {
    ...
    echo "  ~/.freedvtnc2.conf (created)"
    echo "  ~/.reticulum/config (HF interface added)"
    echo ""
    echo "Installed:"
